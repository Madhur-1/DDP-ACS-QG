...
# ##
parser.add_argument(
    "--debug_mode", action="store_true", help="Whether to use debug_mode"
)

parser.add_argument("--do_test", action="store_true", help="Whether to run testing.")
parser.add_argument(
    "--context_question_answer_file", type=str, default="", help="File path."
)
parser.add_argument(
    "--context_question_answer_score_file", type=str, default="", help="File path."
)
parser.add_argument(
    "--context_question_answer_columns", nargs="*", type=int, default=[0, 1, 2]
)
...
def train(args, train_dataset, model, tokenizer):
    ...
    print("weihaojie debug\ttrain success!")
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        eval_dataset, examples = load_and_cache_examples(
        ...
        with open(output_test_file, "w", encoding="utf8") as writer:
    ...
    print("weihaojie debug\tevaluate success!")
    return results


def get_entailment_score(
    args,
    contexts_questions_answers_file,
    context_question_answer_columns,
    output_file,
    model,
    tokenizer,
):
    ...
    questions = []
    answers = []
    print("context_question_answer_columns is:   ", context_question_answer_columns)
    with open(contexts_questions_answers_file, "r", encoding="utf8") as infile:
        lines = infile.readlines()
        for line in lines:
            line_split = str(line).rstrip().split("\t")
            contexts.append(line_split[context_question_answer_columns[0]])
            questions.append(line_split[context_question_answer_columns[1]])
            answers.append(line_split[context_question_answer_columns[2]])
    infile.close()

    with open("./dev.tsv", "w", encoding="utf8") as writer:
        writer.write("Quality\t#1 ID\t#2 ID\t#1 String\t#2 String\n")
        for i, context in enumerate(contexts):
            question, answer = questions[i], answers[i]
            str1, str2 = question + " " + answer, context
            writer.write("{}\t{}\t{}\t{}\t{}\n".format(1, 2 * i, 2 * i + 1, str1, str2))
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        eval_dataset, examples = load_and_cache_examples_forbb(
            args, eval_task, tokenizer, data_dir="./", evaluate=True
        )
...
def load_and_cache_examples_forbb(args, task, tokenizer, data_dir, evaluate=False):
    examples, features = None, None
    processor = processors[task]()
    output_mode = output_modes[task]
    if 1 == 1:
...
def main(args):
    ...
    if args.do_test:
        ...
        model = model_class.from_pretrained(checkpoint)
        model.to(args.device)
        results = get_entailment_score(
            args,
            args.context_question_answer_file,
            args.context_question_answer_columns,
            args.context_question_answer_score_file,
            model,
            tokenizer,
        )
