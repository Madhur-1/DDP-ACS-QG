...
def st_gumbel_softmax2(logits, device, temperature=1.0, mask=None):
    """
    Given input logits of shape (batch_size, seq_length) where each element
    in logits indicate the un-normalized probability of 1, output tensor of
    shape (batch_size, seq_length), each element is 0 or 1, according to the
    probabilities in logits.
    :param logits: (Tensor) A un-normalized probability values,
                   which has the size (batch_size, seq_length)
    :param temperature: (float) A temperature parameter. The higher
                        the value is, the smoother the distribution is.
    :param mask: (Tensor, optional) If given, it masks the softmax
                 so that indices of '0' mask values are not selected.
                 The size is (batch_size, seq_length).
    :returns: The sampled output, which has the property explained above.
    """
    batch_size, seq_length = logits.shape
    logits0 = torch.zeros(batch_size, seq_length).to(device)
    logits10 = torch.stack(
        [logits, logits0]).transpose(0, 1).transpose(1, 2).view(-1, 2)
    y = st_gumbel_softmax(
        logits10, temperature, mask)[:, 0].reshape(batch_size, seq_length)
    return y
...
if __name__ == "__main__":
    ...
    # test st_gumbel_softmax 2
    print("st_gumbel_softmax2")
    logits = torch.FloatTensor([[-1, 2, -5, 2, -3],
                                [3, -2, 8, -1, 1],
                                [-1, 5, 9, -3, -1]])
    print(st_gumbel_softmax2(logits, device="cpu", temperature=1.0, mask=None))
