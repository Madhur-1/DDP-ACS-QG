...
def get_squad_raw_examples(filename, debug=False, debug_length=20):
    ...
    with codecs.open(filename, encoding="utf-8") as fh:
        ...
        for line in tqdm(lines):
            ...
            token_spans = get_token_char_level_spans(fields[0], fields[0].split())
            ...
            if len(answer_spans) > 1:
                for i in range(len(answer_spans)):
                    new_gap = abs(answer_spans[i][0] - answer_start_in_tokenized_sent)
                    if new_gap < gap:
                        choice = i
                        gap = new_gap
...
def get_spacy_processed_examples(
    config, raw_examples, debug=False, debug_length=20, shuffle=False
):
    """
    ...
    print(("Time of get spaCy processed examples: {}").format(datetime.now() - start))
...
def filter_example(config, example, mode="train"):
    ...
    if mode == "train":
        return (
            len(example["ans_sent_tokens"]) > config.sent_limit
            or len(example["ques_tokens"]) > config.ques_limit
            or (example["y2_in_sent"] - example["y1_in_sent"]) > config.ans_limit
        )
    elif mode == "test":
        return (
            len(example["ans_sent_tokens"]) > config.sent_limit
...
def get_lower_word_counter(word_counter):
    ...
    specials = [
        ...
        "<sos>",
        "<eos>",
    ]  # TODO: revise with common constants
...
def get_updated_counters_by_examples(
    config, counters, examples, increment=1, init=False, finish=False
):
    """
    ...
    config.lower = (
        True  # !!!!!! NOTICE: here we always use lowercase option for our QG project
    )
        ...
        for tag in tags:
            ...
            if tag == "word":
                example[field_id] = spacydoc2wids(
                    example[field + "_doc"], emb_dicts[tag], length_limits[field]
                )
            elif tag == "char":
                ...
                example[field_id] = spacydoc2tagids(
                    example[field + "_doc"], tag, emb_dicts[tag], length_limits[field]
                )
            elif tag in [
                "is_alpha",
                "is_ascii",
...
def build_copy_labels(config, example, emb_dicts, related_words_ids_mat):
    ...
    (
        ...
        switch_soft,
        copy_position_soft,
        input_copied_hard_soft,
    ) = get_copy_labels(
        example["ans_sent_tokens"],
        example["ques_tokens"],
        config.sent_limit,
        config.ques_limit,
        emb_dicts["word"],
...
def build_fqg_features(config, example, emb_dicts, related_words_dict):
    ...
    example["ans_sent_is_clue_hard"] = (
        example["ans_sent_is_overlap"] + example["ans_sent_is_content"] == 2.0
    ).astype(float)

    # get ids for embedding layer
    example["ans_sent_is_content_ids"] = feature2ids(
        example["ans_sent_is_content"],
        emb_dicts["is_content"],
        len(example["ans_sent_doc"]),
        config.sent_limit,
    )
    example["ques_is_content_ids"] = feature2ids(
        example["ques_is_content"],
        emb_dicts["is_content"],
        len(example["ques_doc"]),
        config.ques_limit,
    )

    example["ans_sent_is_clue_hard_ids"] = feature2ids(
        example["ans_sent_is_clue_hard"],
        emb_dicts["is_clue_hard"],
        len(example["ans_sent_doc"]),
        config.sent_limit,
    )
...
def prepro(config):
    ...
    if not config.processed_by_spacy and not config.processed_example_features:
        ...
        print("-- Memory stats after Train load and processing by spacy- ", print_gpu_stats())
        ...
        print("-- Memory stats after Dev load and processing by spacy- ", print_gpu_stats())
        ...
        print("-- Memory stats after Test load and processing by spacy- ", print_gpu_stats())
    ...
    if not config.processed_related_words:
        ...
        save(
            config.related_words_dict_file,
            related_words_dict,
    ...
    if not config.processed_example_features:
        ...
        print("-- Memory stats after Train process example features ", print_gpu_stats())
        ...
        print("-- Memory stats after Dev process example features ", print_gpu_stats())
        ...
        print("-- Memory stats after Test process example features ", print_gpu_stats())
    ...
    write_example(train_examples[2], OUTPUT_PATH + "debug\\train_example.txt")
    write_example(dev_examples[2], OUTPUT_PATH + "debug\\dev_example.txt")
    write_example(test_examples[2], OUTPUT_PATH + "debug\\test_example.txt")
    write_dict(train_meta, OUTPUT_PATH + "debug\\train_meta.txt")
    write_dict(dev_meta, OUTPUT_PATH + "debug\\dev_meta.txt")
    write_dict(test_meta, OUTPUT_PATH + "debug\\test_meta.txt")
    write_dict(related_words_dict, OUTPUT_PATH + "debug\\related_words_dict.txt")
    write_2d_list(
        related_words_ids_mat, OUTPUT_PATH + "debug\\related_words_ids_mat.txt"
    )
...
def write_example(e, filename):
    with codecs.open(filename, mode="w", encoding="utf-8") as fh:
        for k in e:
            if (
                isinstance(e[k], np.ndarray)
                or isinstance(e[k], list)
                or isinstance(e[k], int)
                or isinstance(e[k], float)
...
class QGData(Dataset):
    def __init__(self, config, emb_dicts, examples_file):
        ...
        for example in self.examples:
            ...
            example["copy_position_soft"] = (
                example["copy_position_soft"] * example["switch_soft"]
            )
        ...
        if (
            ...
            assert config.refined_tgt_vocab_limit <= config.refined_src_vocab_limit
            assert config.refined_copy_vocab_limit <= config.refined_tgt_vocab_limit
            ...
            for i in range(self.num):
                ...
                for idx, tgt_word in enumerate(example["ques_tokens"]):
                    ...
                    if (
                        (compare_idx is None)
                        or (compare_idx >= config.refined_copy_vocab_limit)
                        or compare_idx == OOV_idx
                    ):
                        if tgt_word.lower() in example["src_tokens"]:
                            switch[idx] = 1
                            # NOTICE: we can revise here, as tgt_word can show multiple times
                            copy_position[idx] = example["src_tokens"].index(
                                tgt_word.lower()
                            )
                    ...
                    if config.use_refine_copy_tgt or config.use_refine_copy_tgt_src:
                        if (
                            (compare_idx is None)
                            or (compare_idx >= config.refined_tgt_vocab_limit)
                            or compare_idx == OOV_idx
                        ):
                            tgt[idx] = OOV_id
                        else:
                ...
                # refine src ids
                if config.use_refine_copy_src or config.use_refine_copy_tgt_src:
                    c_mask = (
                        example["ans_sent_word_ids"] >= config.refined_src_vocab_limit
                    )
                    self.examples[i]["ans_sent_word_ids"] = (
                        c_mask * OOV_id + (1 - c_mask) * example["ans_sent_word_ids"]
                    )
                    q_mask = example["ques_word_ids"] >= config.refined_src_vocab_limit
                    self.examples[i]["ques_word_ids"] = (
                        q_mask * OOV_id + (1 - q_mask) * example["ques_word_ids"]
                    )
    ...
    def __getitem__(self, idx):
        return (
            self.examples[idx]["ans_sent_word_ids"],
...
def collate(data):
    (
        ...
        ans_sent_is_digit,
        ans_sent_is_digit_ids,
        ans_sent_is_lower,
        ans_sent_is_lower_ids,
        ans_sent_is_punct,
        ans_sent_is_punct_ids,
        ans_sent_is_bracket,
        ans_sent_is_bracket_ids,
        ans_sent_is_stop,
        ans_sent_is_stop_ids,
        ans_sent_like_num,
        ans_sent_like_num_ids,
        ans_sent_is_overlap,
        ans_sent_is_overlap_ids,
        ans_sent_syntactic_edges,
        ans_sent_is_content,
        ans_sent_is_content_ids,
        ans_sent_is_clue_hard,
        ans_sent_is_clue_hard_ids,
        ans_sent_is_clue,
        ans_sent_is_clue_ids,
        ques_word_ids,
        ques_char_ids,
        ques_bpe_ids,
        ques_pos_ids,
    ...
    batch = {}
    batch["ans_sent_word_ids"] = torch.LongTensor(np.array(ans_sent_word_ids))
    batch["ans_sent_char_ids"] = torch.LongTensor(np.array(ans_sent_char_ids))
    batch["ans_sent_bpe_ids"] = torch.LongTensor(np.array(ans_sent_bpe_ids))
    batch["ans_sent_pos_ids"] = torch.LongTensor(np.array(ans_sent_pos_ids))
    batch["ans_sent_ner_ids"] = torch.LongTensor(np.array(ans_sent_ner_ids))
    batch["ans_sent_iob_ids"] = torch.LongTensor(np.array(ans_sent_iob_ids))
    batch["ans_sent_dep_ids"] = torch.LongTensor(np.array(ans_sent_dep_ids))
    batch["ans_sent_answer_iob_ids"] = torch.LongTensor(
        np.array(ans_sent_answer_iob_ids)
    )
    batch["ans_sent_is_digit"] = torch.FloatTensor(np.array(ans_sent_is_digit))
    batch["ans_sent_is_digit_ids"] = torch.LongTensor(np.array(ans_sent_is_digit_ids))
    batch["ans_sent_is_lower"] = torch.FloatTensor(np.array(ans_sent_is_lower))
    batch["ans_sent_is_lower_ids"] = torch.LongTensor(np.array(ans_sent_is_lower_ids))
    batch["ans_sent_is_punct"] = torch.FloatTensor(np.array(ans_sent_is_punct))
    batch["ans_sent_is_punct_ids"] = torch.LongTensor(np.array(ans_sent_is_punct_ids))
    batch["ans_sent_is_bracket"] = torch.FloatTensor(np.array(ans_sent_is_bracket))
    batch["ans_sent_is_bracket_ids"] = torch.LongTensor(
        np.array(ans_sent_is_bracket_ids)
    )
    batch["ans_sent_is_stop"] = torch.FloatTensor(np.array(ans_sent_is_stop))
    batch["ans_sent_is_stop_ids"] = torch.LongTensor(np.array(ans_sent_is_stop_ids))
    batch["ans_sent_like_num"] = torch.FloatTensor(np.array(ans_sent_like_num))
    batch["ans_sent_like_num_ids"] = torch.LongTensor(np.array(ans_sent_like_num_ids))
    batch["ans_sent_is_overlap"] = torch.FloatTensor(np.array(ans_sent_is_overlap))
    batch["ans_sent_is_overlap_ids"] = torch.LongTensor(
        np.array(ans_sent_is_overlap_ids)
    )
    batch["ans_sent_syntactic_edges"] = ans_sent_syntactic_edges
    batch["ans_sent_is_content"] = torch.LongTensor(np.array(ans_sent_is_content))
    batch["ans_sent_is_content_ids"] = torch.LongTensor(
        np.array(ans_sent_is_content_ids)
    )
    batch["ans_sent_is_clue_hard"] = ans_sent_is_clue_hard
    batch["ans_sent_is_clue_hard_ids"] = ans_sent_is_clue_hard_ids
    batch["ans_sent_is_clue"] = torch.FloatTensor(np.array(ans_sent_is_clue))
    batch["ans_sent_is_clue_ids"] = torch.LongTensor(np.array(ans_sent_is_clue_ids))
    batch["ques_word_ids"] = torch.LongTensor(np.array(ques_word_ids))
    batch["ques_char_ids"] = torch.LongTensor(np.array(ques_char_ids))
    batch["ques_bpe_ids"] = torch.LongTensor(np.array(ques_bpe_ids))
    batch["ques_pos_ids"] = torch.LongTensor(np.array(ques_pos_ids))
    batch["ques_ner_ids"] = torch.LongTensor(np.array(ques_ner_ids))
    batch["ques_iob_ids"] = torch.LongTensor(np.array(ques_iob_ids))
    batch["ques_dep_ids"] = torch.LongTensor(np.array(ques_dep_ids))
    batch["ques_is_digit"] = torch.FloatTensor(np.array(ques_is_digit))
    batch["ques_is_digit_ids"] = torch.LongTensor(np.array(ques_is_digit_ids))
    batch["ques_is_lower"] = torch.FloatTensor(np.array(ques_is_lower))
    batch["ques_is_lower_ids"] = torch.LongTensor(np.array(ques_is_lower_ids))
    batch["ques_is_punct"] = torch.FloatTensor(np.array(ques_is_punct))
    batch["ques_is_punct_ids"] = torch.LongTensor(np.array(ques_is_punct_ids))
    batch["ques_is_bracket"] = torch.FloatTensor(np.array(ques_is_bracket))
    batch["ques_is_bracket_ids"] = torch.LongTensor(np.array(ques_is_bracket_ids))
    batch["ques_is_stop"] = torch.FloatTensor(np.array(ques_is_stop))
    batch["ques_is_stop_ids"] = torch.LongTensor(np.array(ques_is_stop_ids))
    batch["ques_like_num"] = torch.FloatTensor(np.array(ques_like_num))
    batch["ques_like_num_ids"] = torch.LongTensor(np.array(ques_like_num_ids))
    batch["ques_is_overlap"] = torch.FloatTensor(np.array(ques_is_overlap))
    batch["ques_is_overlap_ids"] = torch.LongTensor(np.array(ques_is_overlap_ids))
    batch["ques_is_content"] = torch.LongTensor(np.array(ques_is_content))  # !!!!
    batch["ques_is_content_ids"] = torch.LongTensor(np.array(ques_is_content_ids))
    batch["ques_type_id"] = torch.LongTensor(np.array(ques_type_id))
    batch["id"] = torch.LongTensor(np.array(id))
    batch["y1_in_sent"] = torch.LongTensor(np.array(y1_in_sent))
    batch["y2_in_sent"] = torch.LongTensor(np.array(y2_in_sent))
    batch["switch"] = torch.FloatTensor(np.array(switch))
    batch["copy_position"] = torch.LongTensor(np.array(copy_position))
    batch["switch_oov"] = torch.FloatTensor(np.array(switch_oov))
    batch["copy_position_oov"] = torch.LongTensor(np.array(copy_position_oov))
    batch["switch_soft"] = torch.FloatTensor(np.array(switch_soft))
    batch["copy_position_soft"] = torch.LongTensor(np.array(copy_position_soft))
    batch["tgt"] = torch.LongTensor(np.array(tgt))
