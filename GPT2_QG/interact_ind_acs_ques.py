...
from .train_ind_acs import (SPECIAL_TOKENS,
                            build_acsq_only_input_from_segments_ques,
                            build_para_only_input_from_segments_ques)
...
def sample_sequence(inst, tokenizer, model, args, para_cache):
    ...
    inst['original_question'] = []  # when generate, if no question, we can set question = []
    inst['question'] = []

    # Ignore the paragraph while building the input instance and token type ids
    instance, _ = build_acsq_only_input_from_segments_ques(inst, tokenizer, with_eos=False)  # as this is used for generate, with_eos is False.
...
def run():
    ...
    final_output_dict = {'pid':[], 'sid':[], "question":[], "answer":[], "paragraph":[],
                    "clue":[], 'ques_type':[]}
    para_cache = {
        "index": None,
        "hidden_states": None
    ...
    question_number = 0
    start = datetime.now()
    for inst in tqdm.tqdm(data.T.to_dict().values()):  # !!! each inst contains one different paragraph
        try:
            with torch.no_grad():
                ...
                if para_index != para_cache["index"]:
                    ...
                    instance, _ = build_para_only_input_from_segments_ques(inst, tokenizer)  # !!!! this is designed to re-use paragraph.
                    input_ids = torch.tensor(instance['input_ids'], device=args.device).unsqueeze(0)
                    token_type_ids = torch.tensor(instance['token_type_ids'], device=args.device).unsqueeze(0)
            ...
            final_output_dict['question'].append(generated_question)
            ...
            final_output_dict['ques_type'].append(tokenizer.decode(inst['style']))


            question_number += 1

            if len(para_index) % args.save_freq == 0:
