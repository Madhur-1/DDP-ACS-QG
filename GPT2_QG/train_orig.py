...
def train():
    ...
    if args.local_rank in [-1, 0]:
        ...
        checkpoint_handler = Checkpoint(
            to_save,
            DiskSaver(args.output_dir+'/gpt_checkpoint/', create_dir=True),
            n_saved=3,
            global_step_transform=global_step_from_engine(trainer),
        )

        # checkpoint_handler = ModelCheckpoint(args.output_dir, 'checkpoint', save_interval=1, n_saved=3, require_empty=False)  # !!!NOTICE: if fill exist, it will report error. set require_empty=False can avoid this.
        trainer.add_event_handler(Events.EPOCH_COMPLETED, checkpoint_handler)  # "getattr" take care of distributed encapsulation
    ...
    # print(checkpoint_handler.last_checkpoint)
    # if not checkpoint_handler.last_checkpoint:
    #     checkpoint = torch.load("/storage/home/madhurjindal/ACS-QG/file/QG/gpt2_question_generation/checkpoint_mymodel_1.pth", map_location=args.device)
    # else:
    #     checkpoint = torch.load(checkpoint_handler.last_checkpoint, map_location=args.device) 
    # Checkpoint.load_objects(to_load={'model': getattr(model, 'module', model)}, checkpoint=checkpoint)
