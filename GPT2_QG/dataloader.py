...
def get_processed_examples(raw_examples, debug=False, debug_length=20, shuffle=False):
    ...
    for e in tqdm(raw_examples):
        ...
        if len(clue_token_position) > 0 and clue_tokenized_text is not None:
            ...
            if (
                len(spans) > clue_end_token_idx
            ):  # !!! NOTICE: sometimes we have len(spans) <= clue_end_token_idx. Need further debug...
...
def get_augmented_sents_examples(
...
def get_dataset(
    ...
    # Load question data
    if dataset_cache and os.path.isfile(
        dataset_cache
    ):  #!!! NOTICE: make sure dataset_cache is correct version.
        print("Load tokenized dataset from cache at %s", dataset_cache)
        data = torch.load(dataset_cache)
        return data

    data = get_positional_dataset_from_file(
        tokenizer, file=path, filetype=filetype, debug=debug, debug_length=debug_length
    )
...
def get_positional_dataset_from_file(
    tokenizer, file, filetype="squad", debug=False, debug_length=20
):
    ...
    for inst in tqdm(data):
        ...
        tokenized_ans_prefix = tokenizer.tokenize(
            inst["paragraph"][: inst["answer_position"]]
        )

        if clue_exist:
            tokenized_clue = tokenizer.tokenize(inst["clue"])
            tokenized_clue_prefix = tokenizer.tokenize(
                inst["paragraph"][: inst["clue_position"]]
            )
        else:
            tokenized_clue = []
        ...
        inst["answer_position_tokenized"] = get_position(
            inst["paragraph"], inst["answer"], ans_prefix_ids
        )
        ...
        if clue_exist:
            ...
            inst["clue_position_tokenized"] = get_position(
                inst["paragraph"], inst["clue"], clue_prefix_ids
            )
    ...
    print(
        "%d / %d sequences truncated due to positional embedding restriction"
        % (truncated_sequences, len(data))
    )
