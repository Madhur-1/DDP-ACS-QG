...
def sample_sequence_beam_search(inst, tokenizer, model, args, para_cache, beam_size=5):
    ...
    inst['beam_gen'] = [final_generation[:-1] for final_generation in final_generations ]
...
def run():
    ...
    question_number = 0
    start = datetime.now()
    for inst in data.T.to_dict().values():  # !!! each inst contains one different paragraph
