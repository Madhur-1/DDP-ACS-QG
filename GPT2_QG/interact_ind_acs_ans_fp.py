...
from .train_ind_acs import (
    SPECIAL_TOKENS,
    SPECIAL_TOKENS_WO_CSQ,
...
def beam_search(
    args, special_tokens_ids, model, token_type, para_cache, instance, beam_size=5
):
    def give_probs(prev, past, instance):
        ...
        model_out = model(
            input_ids, token_type_ids=token_type_ids, past_key_values=past
        )
    ...
    for i in range(args.max_length):
        ...
        curr_past = []
        generations = [
            generations[i] for i in range(beam_size) if i not in aborted_indices
        ]
        ...
        if i == 0:
            probs, past = give_probs(
                prev=None, past=para_cache["hidden_states"], instance=instance
            )
        ...
        for j in range(beam_size):
            probs, past = give_probs(
                prev=top_prevs[j], past=top_pasts[j], instance=instance
            )
...
def sample_sequence_beam_search(inst, tokenizer, model, args, para_cache, beam_size=5):
    ...
    inst["original_question"] = inst[
        "question"
    ]  # when generate, if no question, we can set question = []

    # Ignore the paragraph while building the input instance and token type ids
    instance, _ = build_acsq_only_input_from_segments(
        inst, tokenizer, with_eos=False
    )  # as this is used for generate, with_eos is False.
    ...
    final_generations, final_probs = beam_search(
        args,
        special_tokens_ids,
        model,
        token_type,
...
def get_closest_match(phrase, paragraph):
    ...
    para_list = [
        " ".join(paragraph.split()[i : i + len_phrase])
        for i in range(len(paragraph.split()) - len_phrase + 1)
    ]
...
def get_positional_dataset_from_beam_output(tokenizer, generation, original_paragraph):
    ...
    if beam_gen_position == -1:
        ...
        print(
            f"+++ Got closest match: {exact_match_generation} for {generation} in {original_paragraph}"
        )
    ...
    tokenized_generation_prefix = tokenizer.tokenize(
        original_paragraph[:beam_gen_position]
    )
    ...
    curr_inst["answer_position_tokenized"] = get_position(
        curr_inst["paragraph"], curr_inst["answer"], generation_prefix_ids
    )
...
def run():
    ...
    parser.add_argument(
        "--filename", type=str, default="", help="File to use for decoding"
    )
    parser.add_argument(
        "--filecache", type=str, default="", help="Cache File to use for decoding"
    )
    parser.add_argument(
        "--no_sample",
        action="store_true",
    ...
    parser.add_argument(
        "--data_type", default="augmented_sents", type=str, help="data type"
    )
    parser.add_argument(
        "--output_file",
    ...
    data = get_dataset(
        ...
        args.filecache,
        args.filename,
        filetype=args.data_type,
        debug=args.debug,
    )
    print(
        ("Time of get_positional_dataset_from_file: {}").format(datetime.now() - start)
    )
    ...
    for inst in data:  # !!! each inst contains one different paragraph
        try:
            ...
            para_index = inst[
                "para_index"
            ]  # !!! NOTICE: we can use our sid as this when generate new questions
            if para_index != para_cache["index"]:
                with torch.no_grad():
                    ...
                    instance, _ = build_para_only_input_from_segments(
                        inst, tokenizer
                    )  # !!!! this is designed to re-use paragraph.
                    input_ids = torch.tensor(
                        instance["input_ids"], device=args.device
                    ).unsqueeze(0)
                    token_type_ids = torch.tensor(
                    ...
                    for _ in range(10):
                        try:
                            output = sample_sequence(
                                inst, tokenizer, model, args, para_cache
                            )
                            if output["question"] not in beam_ans:
                                beam_ans.append(output["question"])
                            # output = sample_sequence_beam_search(inst, tokenizer, model, args, para_cache, beam_size=6)

                        except (
                            Exception
                        ) as e:  # !!! NOTICE: sometimes (very rare case) the sample_sequence got size mismatch in modeling_gpt2
                            print(traceback.format_exc())
                            continue

                original_paragraph = tokenizer.decode(output["paragraph"])
                beam_generations = tokenizer.batch_decode(
                    beam_ans, skip_special_tokens=True
                )
                beam_generations = [
                    generation
                    for generation in beam_generations
                    if (
                        check_presence(generation, original_paragraph)
                        or set(generation.split()).issubset(
                            set(original_paragraph.split())
                        )
                    )
                ]
                original_answer = tokenizer.decode(
                    output["answer"], skip_special_tokens=True
                )
                ...
                for ind, generation in enumerate(beam_generations):
                    curr_inst, generation = get_positional_dataset_from_beam_output(
                        tokenizer, generation, original_paragraph
                    )
                    generated_inst["paragraph"].append(curr_inst["paragraph"])
                    generated_inst["answer"].append(curr_inst["answer"])
                    generated_inst["answer_position_tokenized"].append(
                        curr_inst["answer_position_tokenized"]
                    )
                    ...
                    if int(question_number) % args.save_freq == 0:
                        ...
                        pd.DataFrame(final_output_dict).to_csv(
    ...
    print(
        ("Time of generate {} beam_samples: {}").format(
            question_number, datetime.now() - start
        )
    )
