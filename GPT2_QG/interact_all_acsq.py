...
from .train_all_acsq import (SPECIAL_TOKENS, SPECIAL_TOKENS_WO_CSQ,
...
def sample_sequence(inst, tokenizer, model, args, para_cache):
    ...
    for i in range(args.max_length):
        ...
        if i < args.min_length and prev.item() in special_tokens_ids_wo_csq:
            while prev.item() in special_tokens_ids_wo_csq:
                prev = torch.multinomial(probs, num_samples=1)

        if prev.item() in special_tokens_ids_wo_csq:
            break
...
def run():
    ...
    parser.add_argument("--max_length", type=int, default=75, help="Maximum length of the output utterances")
    ...
    final_output_dict = {'pid':[], 'sid':[], "question":[], "answer":[], "paragraph":[],
                    "ans_start":[], "clue":[], 'clue_start':[], 'ques_type':[]}
    para_cache = {
        "index": None,
        "hidden_states": None
    }
    ...
    for inst in tqdm.tqdm(data):  # !!! each inst contains one different paragraph
        try:
            ...
            generated_question = tokenizer.decode(output['question'], skip_special_tokens=False)
            ...
            final_output_dict['ans_start'].append(original_ans_position)
