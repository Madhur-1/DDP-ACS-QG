...
class FQG(nn.Module):
    def __init__(self, config, emb_mats, emb_dicts, dropout=0.1):
        ...
        if self.config.use_style_info:
            ...
            self.style_emb_mat = nn.Parameter(
                torch.randn(self.config.num_question_style, style_embedding_dim)
            ).to(DEVICE)
        ...
        if self.config.copy_type in ["soft", "soft-oov"]:
            ...
            for i in range(self.predict_size):
                related_ids = [i for i in self.related_words_ids_mat[i] if i != -1][
    ...
    def forward(self, batch):
        ...
        maskS = (
            torch.ones_like(batch["ans_sent_word_ids"]) * self.PAD
        ...
        if self.config.use_vocab_mask:
            ...
            wids_to_mask = batch["ans_sent_word_ids"] * torch.LongTensor(
                batch["ans_sent_is_content"]
            ).to(
                DEVICE
            )  # NOTICE!!! memory problem?
            ...
            for ii in range(wids_to_mask.shape[0]):
                ...
                vocab_mask[ii][
                    [
                        SPECIAL_TOKEN2ID["<pad>"],
                        SPECIAL_TOKEN2ID["<oov>"],
                        SPECIAL_TOKEN2ID["<sos>"],
                        SPECIAL_TOKEN2ID["<eos>"],
                    ]
                ] = 1

        # embedding input
        input_emb = self.enc_embedder(batch, "ans_sent", self.enc_emb_tags).transpose(
            1, 2
        )
        ...
        if self.config.copy_type in ["soft", "soft-oov"]:
            ...
            soft_cp_prob = self.related_words_mask[
                src.transpose(0, 1).repeat(c_out.size(0), 1)
            ] * c_out_prob.unsqueeze(1).transpose(1, 2).repeat(1, 1, self.predict_size)
            soft_cp_prob = soft_cp_prob.sum(1)
            soft_cp_prob = soft_cp_prob / soft_cp_prob.sum(1).unsqueeze(1).repeat(
                1, self.predict_size
            )
            ...
            new_g_prob_t = g_out_prob * (
                copyGateOutputs[:, 1].unsqueeze(1).expand_as(g_out_prob)
            ) + soft_cp_prob * (
                copyGateOutputs[:, 2].unsqueeze(1).expand_as(g_out_prob)
            )

            soft_cp_prob = soft_cp_prob.view(
                g_prob_t.shape
            )  # prob of soft-copied words
            new_g_prob_t = new_g_prob_t.view(
                g_prob_t.shape
            )  # prob of both generated and soft-copied words
    ...
    def buildTargetTokens(self, pred, src, isCopy, copyPosition, attn):
        ...
        for i in range(len(tokens)):
            if isCopy[i]:
                ...
                )  # NOTICE: Beam search append input words after vocab words
        ...
        for i in range(len(tokens)):
            if tokens[i] == "<oov>":
                ...
                tokens[i] = (
                    src[maxIndex.item()].encode("utf8").decode("utf8")
                )  # NOTICE: added item() here..
        return tokens
    ...
    def translate_batch(self, batch):
        ...
        dec_hidden = dec_hidden.unsqueeze(0).data.repeat(
            1, beam_size, 1
        )  # NOTICE   any bug here ?????
        att_vec = self.make_init_att(context)
        padMask = (
            src.data.eq(self.dicts["word"]["<pad>"])
            .transpose(0, 1)
            .unsqueeze(0)
        ...
        beam = [
            BeamSearcher(beam_size) for k in range(batch_size)
        ]  # each sample has a beam searcher
        batchIdx = list(range(batch_size))
        remainingSents = (
            batch_size  # count how many remaining samples need to generate output
        )
        remainingSents_batch_ids = list(range(batch_size))

        # decode over each time step
        for i in range(
            self.config.sent_limit
        ):  # NOTICE: !!! maximum output length: sent_limit
            # Prepare decoder input.
            input = (
                torch.stack(  # [1, remainingSents * beam_size]
            ...
            output_emb = self.dec_embedder.embs["word"](
                input
            )  # [1, remainingSents * beam_size, word_emb_dim]
            ...
            (
                ...
                att_vec,
            )  # NOTICE: !!! debug mode, the word emb don't have 20000 unique words, therefore, it will cause index out-of-range error.
            ...
            if self.config.copy_type in ["soft", "soft-oov"]:
                ...
                if self.config.use_vocab_mask:
                    g_out_prob = (
                        g_out_prob
                        * vocab_mask[remainingSents_batch_ids]
                        .repeat((beam_size, 1))
                        .to(DEVICE)
                        + 1e-8
                    )  # NOTICE: make sure mask matches!!!
                soft_cp_prob = self.related_words_mask[
                    src.transpose(0, 1)[remainingSents_batch_ids].repeat(beam_size, 1)
                ] * c_outputs.squeeze(0).unsqueeze(1).transpose(1, 2).repeat(
                    1, 1, self.predict_size
                )
                soft_cp_prob = soft_cp_prob.sum(1)
                soft_cp_prob = soft_cp_prob / soft_cp_prob.sum(1).unsqueeze(1).repeat(
                    1, self.predict_size
                ...
                c_predict = torch.log(
                    c_outputs
                    * (copyGateOutputs[:, 0].unsqueeze(1).expand_as(c_outputs))
                )  # NOTICE: log
            else:
                ...
                if self.config.use_vocab_mask:
                    g_out_prob = (
                        g_out_prob
                        * vocab_mask[remainingSents_batch_ids].repeat((beam_size, 1))
                        + 1e-8
                    )  # NOTICE: make sure mask matches!!!
                g_predict = torch.log(
            ...
            vocab_words_loglike = (
                g_predict.view(beam_size, remainingSents, -1)
                .transpose(0, 1)
                .contiguous()
            )
            copy_words_loglike = (
                c_predict.view(beam_size, remainingSents, -1)
                .transpose(0, 1)
                .contiguous()
            )
            ...
            for b in range(batch_size):
                ...
                idx = batchIdx[
                    b
                ]  # batchIdx is map <beam_idx (1~batch_size), current idx in remained sents>
                if not beam[b].advance(
                    vocab_words_loglike.data[idx],
                    copy_words_loglike.data[idx],
                    attn.data[idx],
                ):  # if not done (done is get <eos>)
                    active += [b]  # record the still active batch index
                    father_idx.append(
                        beam[b].father_beam_idxs[-1]
                    )  # NOTICE: this is very annoying
            ...
            activeIdx = torch.LongTensor([batchIdx[k] for k in active]).to(DEVICE)
            ...
            dec_hidden = (
                dec_hidden.view(-1, dec_hidden.size(2))
                .index_select(0, previous_index.view(-1))
                .view(*dec_hidden.size())
            )
            att_vec = (
                att_vec.view(-1, att_vec.size(1))
                .index_select(0, previous_index.view(-1))
                .view(*att_vec.size())
            )
