...
class BeamSearcher(object):
    def __init__(self, size):
        ...
        self.output_word_ids = [
            torch.LongTensor(size).fill_(SPECIAL_TOKEN2ID["<pad>"]).to(DEVICE)
        ]  # [tensor([0, 0, 0, 0, 0, ..])]
        self.output_word_ids[0][0] = SPECIAL_TOKEN2ID[
            "<sos>"
        ]  # [tensor([2, 0, 0, 0, 0, ..])]
        self.output_word_ids_true = [
            torch.LongTensor(size).fill_(SPECIAL_TOKEN2ID["<pad>"]).to(DEVICE)
        ]
    ...
    def advance(self, vocab_words_loglike, copy_words_loglike, attnOut):
        ...
        """
        vocab_size = vocab_words_loglike.size(
            1
        )  # vocab_words_loglike: [beam_size, vocab_size]
        src_max_len = copy_words_loglike.size(
            1
        )  # copy_words_loglike: [beam_size, src_max_len]
        vocab_plus_input_size = vocab_size + src_max_len
        vocab_and_input_words_loglike = torch.cat(
            (vocab_words_loglike, copy_words_loglike), dim=1
        )  # [beam_size, vocab_size + src_max_len] NOTICE: !!this is using copy_oov, so just concat.
        # vocab_and_input_words_loglike are log prob values of each word in vocab and input sequence.

        if len(self.father_beam_idxs) > 0:
            is_beam_finished = self.output_word_ids[-1].eq(
                SPECIAL_TOKEN2ID["<eos>"]
            )  # 1d binary tensor, indicate which beam is finished.
            if any(is_beam_finished):
                vocab_and_input_words_loglike.masked_fill_(
                    is_beam_finished.unsqueeze(1).expand_as(
                        vocab_and_input_words_loglike
                    ),
                    -float("inf"),
                )  # As the score is log. here inf means the prob of masked position is 0.
                for i in range(self.size):
                    if self.output_word_ids[-1][i] == SPECIAL_TOKEN2ID["<eos>"]:
                        vocab_and_input_words_loglike[i][
                            SPECIAL_TOKEN2ID["<eos>"]
                        ] = 0  # As the score is log. here 0 means the prob of <eos> is 1.
            ...
            cur_output_lens = self.output_lens[
                -1
            ]  # 1d tensor, record the current output length of each beam
            for i in range(self.size):
                cur_output_lens[i] += (
                    0 if self.output_word_ids[-1][i] == SPECIAL_TOKEN2ID["<eos>"] else 1
                )

        # Sum the previous scores.
        if len(self.father_beam_idxs) > 0:
            prev_output_accumulate_loglike = self.output_accumulate_loglikes[-1]
            cur_output_accumulate_loglike = (
                vocab_and_input_words_loglike
                + prev_output_accumulate_loglike.unsqueeze(1).expand_as(
                    vocab_and_input_words_loglike
                )
            )
            # current accumulated scores. here we use +, because scores are log probability.
            cur_output_accumulate_scores = (
                cur_output_accumulate_loglike
                / cur_output_lens.unsqueeze(1).expand_as(cur_output_accumulate_loglike)
            )
            # cur_output_accumulate_scores is cur_output_accumulate_loglike normalized by each beam's output sequence length.
        else:
            self.output_lens.append(
                torch.FloatTensor(self.size).fill_(1).to(DEVICE)
            )  # init length is [1, 1, 1, 1, 1]
            cur_output_accumulate_scores = vocab_and_input_words_loglike[0]
            # init cur_output_accumulate_scores, vocab_and_input_words_loglike[0] [beam_size, vocab_size + max_src_len]

        flat_cur_output_accumulate_scores = cur_output_accumulate_scores.view(
            -1
        )  # so the size is beam_size * (vocab_size + max_src_len)

        (
            best_output_accumulate_scores,
            best_output_accumulate_scores_id,
        ) = flat_cur_output_accumulate_scores.topk(self.size, 0, True, True)
        ...
        # note: we add the // to convert it to a proper int index

        predict = (
            best_output_accumulate_scores_id - father_beam_idx * vocab_plus_input_size
        )
        # predict - 1d tensor, length=beam_size. It indicates the word index (0 ~ vocab_size + max_src_len)
        # of top beam_size predictions.
        isCopy = predict.ge(
            torch.LongTensor(self.size).fill_(vocab_size).to(DEVICE)
        ).long()  # NOTICE
        ...
        final_predict = (
            predict * (1 - isCopy) + isCopy * SPECIAL_TOKEN2ID["<oov>"]
        )  # NOTICE: !!!!!!! This is used for only hard-oov copy.
        ...
        if len(self.father_beam_idxs) > 0:
            ...
            self.output_accumulate_loglikes.append(
                cur_output_accumulate_loglike.view(-1).index_select(
                    0, best_output_accumulate_scores_id
                )
            )
        ...
        self.output_word_ids.append(
            final_predict
        )  # output_word_ids stores the predicted word ids. copied words id replaced by oov id
        self.output_word_ids_true.append(
            predict
        )  # nexYs_true stores the predicted word ids. copied words id not replaced by oov id
        self.isCopy.append(isCopy)
        self.attn.append(
            attnOut.index_select(0, father_beam_idx)
        )  # store the top beam_size output's attn weight on each step
