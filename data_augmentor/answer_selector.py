...
def _post(orig_chunklist):
    ...
    for (
        chunk
    ) in cand_chunks:  # We need to select from the already filtered cand_chunks list
        label, leaves, st, ed = chunk
...
def select_answers(context, processed_by_spacy=False):
    ...
    try:
        tree = PARSER.parse(
            context
        )  # TODO: if the context is too long, it will cause error.
    except:
        pass
...
if __name__ == "__main__":
    sent_list = ["Madhur is a boy. He is 22 years old.\n"]
