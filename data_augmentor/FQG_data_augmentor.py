"""Dirty code for debugging answer_selector"""
import sys

from tqdm import tqdm

sys.path.append(r"E:\WorkProjects\DDP\PaperImplemetations\ACS-QG")
...
def get_chunks(sentence, doc=None):
    ...
    try:
        tree = PARSER.parse(
            sentence
        )  # NOTICE: when sentence is too long, it will have error.
    except:
        pass
    if doc is None:
        doc = NLP(sentence)
...
def get_clue_info(
    ...
    for chunk in chunklist:
        ...
        candidate_clue_is_content = [
            int(w.lower() not in FUNCTION_WORDS_LIST) for w in candidate_clue
        ]
        candidate_clue_lemmas = sent_lemmas[chunk[3] : chunk[4] + 1]
        candidate_clue_content_lemmas = [
            candidate_clue_lemmas[i]
            for i in range(len(candidate_clue_lemmas))
            if candidate_clue_is_content[i] == 1
        ]
        candidate_clue_lemmas_in_ques = [
            candidate_clue_lemmas[i]
            for i in range(len(candidate_clue_lemmas))
            if candidate_clue_lemmas[i] in ques_lemmas
        ]
        candidate_clue_content_lemmas_in_ques = [
            candidate_clue_content_lemmas[i]
            for i in range(len(candidate_clue_content_lemmas))
            if candidate_clue_content_lemmas[i] in ques_lemmas
        ]

        candidate_clue_tokens_in_ques = [
            candidate_clue[i]
            for i in range(len(candidate_clue))
            if candidate_clue[i].lower() in ques_tokens
        ]
        candidate_clue_content_tokens = [
            candidate_clue[i]
            for i in range(len(candidate_clue))
            if candidate_clue_is_content[i] == 1
        ]
        candidate_clue_content_tokens_in_ques = [
            candidate_clue_content_tokens[i]
            for i in range(len(candidate_clue_content_tokens))
            if candidate_clue_content_tokens[i].lower() in ques_tokens
        ]
        candidate_clue_content_tokens_in_ques_soft = (
            candidate_clue_content_tokens_in_ques  # !!!! TODO: soft in.
        )

        score = 0
        if (
            (
                len(candidate_clue_lemmas_in_ques) == len(candidate_clue_lemmas)
                or len(candidate_clue_tokens_in_ques) == len(candidate_clue)
            )
            and sum(candidate_clue_is_content) > 0
            and candidate_clue[0].lower() not in NOT_BEGIN_TOKENS_FOR_ANSWER_CLUE
        ):
...
def get_answer_clue_style_info(
    sentence,
    question,
    answer,
    answer_start,
...
def get_sample_probs(
    filename,
    filetype,
    save_dataset_info_file=None,
    save_sample_probs_file=None,
    sent_limit=100,
    ques_limit=50,
    answer_limit=30,
    is_clue_topN=20,
...
):  #!!!!!!!!!!!!! maybe 20 makes the prob be big for clue_dep > 20... set it as a big value like 100?
    """
    P(a, c, s) = p(a) * p(c|a) * p(s|c, a)
               = p(a|a_tag, a_length) * p(c|c_tag, dep_dist) * p(s|a_tag)
    """
    ...
    sample_probs = {"a": ala_tag_a_length, "c|a": clc_tag_dep_dist, "s|c,a": sla_tag}
...
def select_question_types(
    sample_probs, selected_answer, num_sample_style=2, max_sample_times=20
):
    (
        answer_text,
        char_st,
    ...
    for ans in sampled_answers:
        ...
        info = {
            "answer": {
                ...
                "answer_chunk_tag": answer_pos_tag,
            },
            "styles": None,
            "clues": None,
        }
...
if __name__ == "__main__":
    ...
    filetype = "squad"
    save_dataset_info_file = (
        "Datasets/processed/SQuAD1.1-Zhou/squad_ans_clue_style_info.pkl"
    )
