...
def sentences2augmented_sentences(
    input_path,
    output_path,
    start_index,
    end_index,
...
def main(args):
    ...
    DATA_PATH = "/scratch/scratch8/madhurjindal/ACS-QG-Scratch/Datasets/"

    DATA_ACS_INFO_FILE_PATH = (
        DATA_PATH + "processed/SQuAD1.1-Zhou/squad_ans_clue_style_info.pkl"
    )
    SAMPLE_PROBS_FILE_PATH = (
        DATA_PATH + "processed/SQuAD1.1-Zhou/squad_sample_probs.pkl"
    )
    ...
    if (
        not os.path.isfile(SAMPLE_PROBS_FILE_PATH)
        or args.not_processed_sample_probs_file
    ):
